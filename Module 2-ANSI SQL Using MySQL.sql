...
-- Question 1: Show a list of all upcoming events a user is registered for in their city, sorted by date.
...
-- Question 2:Identify events with the highest average rating, considering only those that have received at least 10 feedback submissions.   
 
 SELECT e.event_id, e.title, COUNT(f.feedback_id) AS total_feedbacks, AVG(f.rating) AS average_rating
FROM 
    Events e
JOIN 
...
ORDER BY 
    average_rating DESC;
-- Question 3:Retrieve users who have not registered for any events in the last 90 days. 
...
-- Question 4: Count how many sessions are scheduled between 10 AM to 12 PM for each event.
...
-- Question 5:List the top 5 cities with the highest number of distinct user registrations. 
...
-- Question 6:Generate a report showing the number of resources (PDFs, images, links) uploaded for each  event.
...
--  Question 7:List all users who gave feedback with a rating less than 3, along with their comments and associated event names.
...
-- Question 8:Display all upcoming events with the count of sessions scheduled for them. 

SELECT e.event_id, e.title AS event_title, e.start_date, e.end_date, COUNT(s.session_id) AS session_count
FROM Events e
...
-- Question 9:For each event organizer, show the number of events created and their current status (upcoming, completed, cancelled).
...
-- Question 10:Identify events that had registrations but received no feedback at all.
...
HAVING COUNT(r.registration_id) > 0 
 ...
 -- Questionn 11:Find the number of users who registered each day in the last 7 days.
...
-- Question 12:List the event(s) with the highest number of sessions.
...
-- Question 13:Calculate the average feedback rating of events conducted in each city.
...
-- Question 14:List top 3 events based on the total number of user registrations.
...
-- Question 15:Identify overlapping sessions within the same event (i.e., session start and end times that conflict). 
...
-- Question 16:Find users who created an account in the last 30 days but haven’t registered for any events. 
...
-- Question 17:Identify speakers who are handling more than one session across all events.
...
-- Question 18:List all events that do not have any resources uploaded.
...
-- Question 19:For completed events, show total registrations and average feedback rating. 

SELECT e.event_id, e.title AS event_title, COUNT(DISTINCT r.registration_id) AS total_registrations, AVG(f.rating) AS average_feedback_rating
FROM Events e
LEFT JOIN Registrations r ON e.event_id = r.event_id
LEFT JOIN Feedback f ON e.event_id = f.event_id
...
-- Question 20:For each user, calculate how many events they attended and how many feedbacks they submitted. 
...
-- Question 21:List top 5 users who have submitted the most feedback entries. 
...
-- Questions 22:Detect if a user has been registered more than once for the same event.
SELECT user_id, event_id, COUNT(*) AS registration_count
FROM Registrations
GROUP BY user_id, event_id
HAVING registration_count > 1;

-- Question 23:Show a month-wise registration count trend over the past 12 months.
...
-- Question 24:Compute the average duration (in minutes) of sessions in each event.
SELECT e.event_id, e.title AS event_title,
...
-- Question 25:List all events that currently have no sessions scheduled under them.
SELECT e.event_id, e.title AS event_title
FROM Events e
LEFT JOIN Sessions s ON e.event_id = s.event_id
